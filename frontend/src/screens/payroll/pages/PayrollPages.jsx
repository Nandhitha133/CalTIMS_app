...
export const EmployeePayrollProfiles = () => {
   ...
   const { data: users } = useQuery({
      queryKey: ['users', 'decrypted'],
      queryFn: () => userAPI.getAll({ limit: 1000, decryptPII: true }).then(res => res.data.data)
   });
...
export const SalaryStructures = () => {
   ...
   const { data: usersData } = useQuery({
      queryKey: ['usersList', 'decrypted'],
      queryFn: () => userAPI.getAll({ decryptPII: true }).then(res => res.data?.data || res.data || [])
   });
...
export const PayslipGeneration = () => {
   ...
   const { data: usersData } = useQuery({
      queryKey: ['usersListForPayslips', 'decrypted'],
      queryFn: () => userAPI.getAll({ limit: 1000, decryptPII: true }).then(res => res.data?.data || res.data || [])
   });
